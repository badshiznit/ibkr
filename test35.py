...
def simulate_trades(df, crossovers, risk_reward_ratio):
    ...
    for crossover in crossovers:
        ...
        # Long trade
        stop_loss_long = entry_price - (STOP_LOSS_TICKS * TICK_SIZE)
        take_profit_long = entry_price + (STOP_LOSS_TICKS * risk_reward_ratio * TICK_SIZE)
        in_long_trade = True
        ...
        for index, row in df.loc[df.index >= entry_time].iterrows():
            if in_long_trade:
                if row['low'] <= stop_loss_long:
                    ...
                    in_long_trade = False
                elif row['high'] >= take_profit_long:
                    trades.append({
                        'entry_time': entry_time,
                        'entry_price': entry_price,
                        'stop_loss': stop_loss_long,
        ...
        # Short trade
        stop_loss_short = entry_price + (STOP_LOSS_TICKS * TICK_SIZE)
        take_profit_short = entry_price - (STOP_LOSS_TICKS * risk_reward_ratio * TICK_SIZE)
        in_short_trade = True
        ...
        for index, row in df.loc[df.index >= entry_time].iterrows():
            if in_short_trade:
                ...
                elif row['low'] <= take_profit_short:
                    trades.append({
                        'entry_time': entry_time,
                        'entry_price': entry_price,
                        'stop_loss': stop_loss_short,
...
def main():
    ...
    # Connexion à l'API Interactive Brokers
    ib = connect_ib()
    if ib is None:
        return

    # Récupération des derniers jours de trading
    ...
    # Calcul des données de trading
    trading_data = calculate_trading_data(df)
    trading_data = add_previous_day_levels(trading_data)

    # Utilisation des derniers jours de trading pour la simulation
    ...
    # Calcul du résumé des performances
    ...
    if trades:
        ...
        header = "{:<20} {:<10} {:<10} {:<10} {:<10} {:<10} {:<20} {:<15} {:<10} {:<15} {:<15} {:<15} {:<10} {:<10} {:<10} {:<10}".format(
            "EntryTime", "Price", "SL", "TP", "Direction", "Outcome", "CloseTime", "CrossLevel", "CrossValue", "OpenPosPct", "OpenVsPrvClose", "OpenVsMdn8t", "RSI", "ATR", "EMA", "Color"
        )
        print(header)
        print("-" * len(header))
        for trade in trades:
            print("{:<20} {:<10.2f} {:<10.2f} {:<10.2f} {:<10} {:<10} {:<20} {:<15} {:<10.2f} {:<15.2f} {:<15.2f} {:<15.2f} {:<10.2f} {:<10.2f} {:<10.2f} {:<10}".format(
                ...
                trade['open_position_pct'], trade['open_vs_prv_close'], trade['open_vs_mdn8t'], trade['rsi'], trade['atr'], trade['ema'], trade['color']
            ))
